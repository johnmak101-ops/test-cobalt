...
# Use `migrate` (applies packages/contracts/drizzle/*.sql) — NOT `push`, which needs a TTY prompt.
if [ "$RUN_MIGRATIONS" = "1" ]; then
  echo "[entrypoint] applying migrations (drizzle-kit migrate) against $DATABASE_URL ..."
  pnpm --filter @cobalt/contracts exec drizzle-kit migrate || echo "[entrypoint] WARN: migrate failed"
fi

...
2. Triage the list before fixing anything (see below). Fix causes, not symptoms.
3. Re-run `npx tsc --noEmit`. The error count must drop. If it did not, undo that edit and re-triage — you fixed a symptom.
4. Repeat until zero errors remain, then optionally run `npx eslint --fix`.
...
### Triage: a large error count is usually one cause

A few hundred type errors almost never means a few hundred bugs. It usually means one unmet prerequisite, with the compiler faithfully reporting every place that depends on it. Fixing those places one at a time is wasted work: each edit is wrong, the count does not fall, and the real cause stays untouched.

Group the errors before you fix any of them:

- `TS2307 Cannot find module` and `TS2304 Cannot find name` are CAUSES. An unresolved import poisons every type that flows from it.
- `TS2339 Property 'x' does not exist on type 'T'` and `TS2345 Argument of type ...` are usually EFFECTS. When many of them name the same type `T`, open the file that declares `T`: if it has an unresolved import, that import is the only real error, and fixing it clears the rest.

So: resolve every unresolved import first, re-run, then re-read what is left. Only the errors that survive that pass are real work.

A module under a path like `generated/`, `.prisma/`, `__generated__/`, or `.svelte-kit/`, or one named in an ORM or codegen config, is produced by a generator. When it is missing, run the generator — the project's own `package.json` scripts (`generate`, `db:generate`, `codegen`, `prepare`, `sync`) are the authority on how. NEVER create, stub, or hand-write a file that a generator owns: it will be overwritten, it will not match the schema, and it hides the real failure. Silencing these errors with `any`, `@ts-ignore`, or a hand-made declaration file is the same mistake.

...
describe('mesh mappers', () => {
  it('maps an active customer; drops inactive; falls back name→code', () => {
    expect(mapCustomer({ CustomerCode: '2TAL', FullNameEn: '2TALL.COM RETAIL LIMITED ', FullNameCh: '', IsActive: true })).toEqual({ code: '2TAL', name: '2TALL.COM RETAIL LIMITED' })
    expect(mapCustomer({ CustomerCode: 'X', FullNameEn: '', FullNameCh: '', IsActive: true })).toEqual({ code: 'X', name: 'X' })
...
describe('MeshClient', () => {
  ...
  it('fetches a token then GETs with Bearer, and caches the token across calls', async () => {
    ...
    expect(await c.customers()).toEqual([{ code: 'A', name: 'Acme' }])
    await c.customers()

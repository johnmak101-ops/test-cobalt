...
export class KyselyShipmentRepository {
  ...
  async findByIds(ids: string[]): Promise<Map<string, Awaited<ReturnType<KyselyShipmentRepository['findById']>>>> {
    const map = new Map<string, Awaited<ReturnType<KyselyShipmentRepository['findById']>>>()
  ...
  async insertLeg(values: Record<string, unknown>) {
    // matchKeys is a JSON nvarchar(max) column — stringify on insert.
    const payload: Record<string, unknown> = { ...values }
    payload.matchKeys = payload.matchKeys != null ? JSON.stringify(payload.matchKeys) : null
    const row = await this.db
  ...
  legsForTracker(status?: string) {
    ...
    if (status) q = q.where('shipments.state', '=', status)
    return q.execute()
  }
  ...
  async milestonesForShipments(ids: string[]) {
    const map = new Map<string, Awaited<ReturnType<KyselyShipmentRepository['milestonesFor']>>>()
    if (!ids.length) return map
  ...
  async documents() {
    const rows = await this.db
      ...
      .orderBy(sql`case when ${KyselyShipmentRepository.receivedAtMaxExpr()} is null then 1 else 0 end`, 'asc')
      .orderBy(KyselyShipmentRepository.receivedAtMaxExpr(), 'desc')
      .select([
        ...
        KyselyShipmentRepository.poNumbersSubquery(),
        KyselyShipmentRepository.receivedAtMaxExpr().as('receivedAt'),
      ])
      .execute()

...
| `architecture/require-module-boundaries` | Feature modules share global `RepositoriesModule`; strict boundaries would be a large restructure. |
| `performance/no-unused-module-exports` | Exports kept for tests, CLI reuse, or future wiring. |
| CLI files (`sync-masters`, `sync-ports`, `migrate-cli`, `seed`) | Outside Nest DI; `process.env` + manual `new` are correct. |
...
```bash
pnpm exec nestjs-doctor . --score --config nestjs-doctor.config.json
```

...
export default function PurchaseOrderDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <span className="text-sm text-text-muted">Loading purchase order...</span>
      </div>
    )
  }
  ...
  if (!po) {
    return (
      <div className="flex h-64 items-center justify-center">
        <span className="text-sm text-text-muted">Purchase order not found</span>
      </div>
    )
  }
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-4 sm:grid-cols-4">
          <div>
            ...
            {po.quantityUnit && (
              <span className="text-xs text-text-muted">{po.quantityUnit}</span>
            )}
          </div>
          <div>
                ...
                {po.linkedShipments.map((shipment) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-mono text-sm text-text-secondary">
                      {shipment.linkedQuantity != null
                        ? `${shipment.linkedQuantity} ${po.quantityUnit ?? ''}`
                        : '—'}
                    </td>
                  </tr>
                ))}

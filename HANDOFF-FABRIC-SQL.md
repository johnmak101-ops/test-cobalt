...
1. **Fabric-deploy verification** — run migrations + a suite pass against a real Fabric SQL DB (Entra auth)
   before prod; `sp_MSforeachtable` paths (test reset, demo wipe) are dev-only by design.
